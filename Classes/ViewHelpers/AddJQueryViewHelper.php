 ...
 */
class Tx_SocialSharingPrivacy_ViewHelpers_AddJQueryViewHelper extends Tx_Fluid_Core_ViewHelper_AbstractTagBasedViewHelper
{
	...
	public function render($altJQueryFile = NULL, $moveToFooter = FALSE) {
		...
		if (T3JQUERY === true) {
			...
			if ($altJQueryFile) {
				Tx_SocialSharingPrivacy_Utility_Div::addCssJsFile(
					$altJQueryFile,
					$moveToFooter
				);
			} else {
				Tx_SocialSharingPrivacy_Utility_Div::renderFlashMessage(
					'jQuery not loaded',
